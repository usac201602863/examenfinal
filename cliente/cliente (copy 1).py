...
def sendALIVE(recibido=False, retain = False):  # EAMA Funcion para publicar
    i = 0
    send=True
    aux = 0
    while send:
        topic="comandos"+'/'+str(Grupo)+'/'+usuario()[0]    # EAMA topic para enviar Alive
        ...
        if(recibido==True): # EAMA si recibe un Ack
            ...
            i=0    # Reinicia el contador de Alives
        elif (i<3):   # EAMA Cuenta 3 periodos de Alive si no recibe ack
            time.sleep(ALIVE_PERIOD) # EAMA Delay normal de 2 segundos
            i+=1    # aumenta el contador de Alives
            aux+=ALIVE_PERIOD
        elif(i>=3 and i<203 and recibido ==False):    # Si no recibio un Ack en 20 periodos continuos
            time.sleep(ALIVE_CONTINUO)  # EAMA Delay de 0.1 segundos
            i+=1
            aux+=ALIVE_CONTINUO
        else:   # EAMA  en caso de no recibir un Ack en 20 segundos
            logging.critical("No se puede establecer conexion con el servidor.")
            logging.critical("Saliendo de la Aplicacion...")
            raise exit(1)   # Excepcion para salir 
